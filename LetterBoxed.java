...
public class LetterBoxed {
    ...
    private boolean isValid(String letter, int wordNum, int charNum) {
        //System.out.println(this.words[wordNum] + letter);;
        if (this.words[wordNum] != "" && this.alreadyUsed(this.words[wordNum] + letter)) {
            return false;
        }

        if (wordNum != 0 && (charNum == 0) && !(letter.equals(this.lastLetter(this.words[wordNum-1])))) {
            return false;
        }

        if ((charNum != 0) && this.onSameSide(letter, this.lastLetter(this.words[wordNum]))) {
            return false;
        }

        if (this.dictionary.hasString(this.words[wordNum] + letter)) {
            return true;
        }
        return false;
    }
     ...
     */
        ...
        private boolean solveRB(int wordNum, int charNum, int maxWords) {
            ...
            if (this.allLettersUsed() && this.dictionary.hasFullWord(this.words[wordNum])) {
                this.printSolution(wordNum);
                return true;
            }

            for (int val = 0; val < this.letters.length; val++) {
                if (this.isValid(this.letters[val], wordNum, charNum)) {
                    //for (int i = 1; i < this.words.length; i++) {
                    //    System.out.println(this.words[i]);
                    //}
                    this.addLetter(this.letters[val], wordNum);
                    if (this.solveRB(wordNum, charNum+1, maxWords)) {
                        return true;
                    } else if(this.dictionary.hasFullWord(this.words[wordNum]) && (this.words[wordNum].length() >= 3)) {
